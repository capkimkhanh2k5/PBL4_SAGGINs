...
class SagsEnv(gym.Env):
    def __init__(self):
        ...
        self.waiting_queue = Queue() #Store requests waiting for resource
        ...
        self.stop_timeout_thread = False  # Flag to stop the thread on env close
        self._timeout_thread = threading.Thread(target=self._manage_request_timeouts, daemon=True)
        self._timeout_thread.start()
    
    def reset(self, seed=None, options=None, request : request = None):
        ...
        if request is not None:
            ...
            self.current_request = self.waiting_queue.get()  # blocks until new request

        #run dijkstra
        time_prv = time.time()
        self.run_dijkstra()
        time_run_dij = time.time() - time_prv;
        self.current_request.dis_QoS['dijkstra_time'] = time_run_dij
    ...
    def run_dijkstra(self):
        ...
        self.current_request.dis_path = []
        self.current_request.dis_QoS = {}

        distances = {node_id: math.inf for node_id in self.network.nodes}
        previous_nodes = {node_id: None for node_id in self.network.nodes}

        path_latency = {node_id: math.inf for node_id in self.network.nodes}
        path_reliability = {node_id: 0.0 for node_id in self.network.nodes}
        path_upLink = {node_id: math.inf for node_id in self.network.nodes}
        path_downLink = {node_id: math.inf for node_id in self.network.nodes}

        pq = [] # HEAP (cost, node_id)
        visited = set()

        #find start nodes can connect from source location
        start_nodes = self.network.find_connectable_nodes_for_location(
            self.current_request.source_location["lat"],
            self.current_request.source_location["lon"],
            self.current_request.source_location.get("alt", 0)
        )
        ...
        for start_node in start_nodes:
            cost = self.calculate_cost(None, start_node)

            if cost != math.inf:
                distances[start_node.id] = cost
                previous_nodes[start_node.id] = "SOURCE_USER"

                loc_u = self.current_request.source_location
                type_u = "user"
                ...
                path_latency[start_node.id] = calc_link_delay_ms(distance, type_u, type_v, self.current_request.type)
                ...
                path_upLink[start_node.id] = min(free_uplink, self.current_request.uplink_required)
                path_downLink[start_node.id] = min(free_downlink, self.current_request.downlink_required)
        ...
        while pq:
            ...
            current_node = self.network.get_node_by_id(current_node_id)
            ...
            neighbors = self.network.find_connectable_nodes(current_node_id)
            for neighbor in neighbors:
                if neighbor.id in visited:
                    continue

                #calculate cost
                cost = self.calculate_cost(current_node, neighbor)
                if cost == math.inf:
                    continue
                ...
                if new_cost < distances[neighbor.id]:
                    ...
                    link_lat = calc_link_delay_ms(dis, type_u, type_v, self.current_request.type)
        ...
        if best_gs_id is not None:
            ...
            self.current_request.dis_path = list(reversed(path))

            gs_node = self.network.get_node_by_id(best_gs_id)
            gs_free_cpu = 0
            gs_free_power = 0
            if gs_node and gs_node.typename == "groundstation":
                gs_free_cpu = min(max(gs_node.resources["cpu"] * MAX_USAGE - gs_node.resources_used["cpu"], 0), self.current_request.cpu_required)
                gs_free_power = min(max(gs_node.resources["power"] * MAX_USAGE - gs_node.resources_used["power"], 0), self.current_request.power_required)
            ...
            self.current_request.dis_QoS = {
                ...
                "hops": len(self.current_request.dis_path),
            }

    #calculate cost djikstra from node u to node v
    def calculate_cost(self, node_u: node, node_v: node):
        if self.current_request.type == ServiceType.EMERGENCY:
            MAX_USAGE = 0.95
        else:
            MAX_USAGE = 0.9
        ...
        if free_uplink < self.current_request.uplink_required * MIN_QOS_COST or free_downlink < self.current_request.downlink_required * MIN_QOS_COST:
        ...
        if node_v.typename == "groundstation":
            ...
            if free_cpu < self.current_request.cpu_required * MIN_QOS_COST or free_power < self.current_request.power_required * MIN_QOS_COST:
                return math.inf  # Không đủ tài nguyên tính toán
        ...
        distance = node_v.calculate_distance(loc_u["lat"], loc_u["lon"], loc_u.get("alt", 0))
        
        lat_cost = calc_link_delay_ms(distance, type_u, type_v, self.current_request.type)
        rel_cost = link_reliability(type_u, type_v, distance)
        
        total = lat_cost / self.current_request.latency_required + (1 - rel_cost) / self.current_request.reliability_required
        
        return total

    
    def _manage_request_timeouts(self, interval: float = 1.0):
        """
        Background thread to manage request timeouts.
        Checks self.connections and releases expired requests.
        """
        while not self.stop_timeout_thread:
            timeout_req = []
            with self._queue_lock:  # Protect connections and groundspace
                # Decrement timeout counters
                for index, req in enumerate(self.connections):
                    req.real_timeout -= 1
                    if req.real_timeout <= 0:
                        timeout_req.append(index)
                
                # Remove expired requests safely (reverse index order)
                for index in reversed(timeout_req):
                    ...
                    for node_id in req.path:
                        node_obj = self.network.get_node_by_id(node_id)
                        if node_obj:
                            node_obj.release_resource(req)
                            # self.log_remove(node_obj, req)  # Optional: log released resources
            time.sleep(interval)
    ...
    def add_external_request(self, new_request):
        """Called by FastAPI to push a new request."""
        loop = asyncio.get_event_loop()
        fut = loop.create_future()          # Future that will eventually hold the result
        self.pending_requests[new_request.request_id] = fut
        self.waiting_queue.put(new_request)  # Non-blocking push
        return fut
    ...
    def is_episode_ended(self):
        if self.steps > self.MAX_STEP:
            return True
        if self.neighbor_ids[0] is None and self.current_node and self.current_node.typename != "groundstation":
            return True
        if self.current_node and self.current_node.typename == "groundstation":
            return True
        return False
            
    def close(self):
        self.stop_timeout_thread = True
    ...
    def _new_request(self):
        ...
        demand_timeout = random.randint(150, 3500)  # episode
    ...
    def get_snapshot(self):
        """
        Returns a lightweight snapshot of the environment's current network and connections,
        consistent with /getenvresources endpoint format.
        Avoids deepcopy and only extracts serializable attributes.
        """
        # Thread-safe read of network and connections
        with self._queue_lock:
            network_ref = self.network
            connections_ref = list(self.connections)

        # ---- Nodes snapshot ----
        nodes_snapshot = []
        for node in network_ref.nodes.values():
            try:
                nodes_snapshot.append({
                    "id": node.id,
                    "type": node.typename,
                    "sat_type": node.type if node.typename == "satellite" else None,
                    "position": dict(node.position),
                    "resources": {
                        "uplink": node.resources.get("uplink"),
                        "downlink": node.resources.get("downlink"),
                        "cpu": node.resources.get("cpu"),
                        "power": node.resources.get("power"),
                    },
                    "resources_used": {
                        "uplink": node.resources_used.get("uplink"),
                        "downlink": node.resources_used.get("downlink"),
                        "cpu": node.resources_used.get("cpu"),
                        "power": node.resources_used.get("power"),
                    },
                })
        ...
        for conn in connections_ref:
            try:
                connections_snapshot.append({
                    "id": conn.request_id,
                    "source_location": dict(conn.source_location),
                    "type": conn.type.value,
                    "uplink_required": conn.uplink_required,
                    "downlink_required": conn.downlink_required,
                    "latency_required": conn.latency_required,
                    "reliability_required": conn.reliability_required,
                    "cpu_required": conn.cpu_required,
                    "power_required": conn.power_required,
                    "packet_size": conn.packet_size,
                    "uplink_allocated": conn.uplink_allocated,
                    "downlink_allocated": conn.downlink_allocated,
                    "latency_actual": conn.latency_actual,
                    "reliability_actual": conn.reliability_actual,
                    "cpu_allocated": conn.cpu_allocated,
                    "power_allocated": conn.power_allocated,
                    "priority": conn.priority,
                    "timeout_demand": conn.demand_timeout,
                    "real_timeout": conn.real_timeout,
                    "direct_sat_support": conn.direct_sat_support,
                    "path": [node_id for node_id in conn.path],
                    "dis_path": [node_if for node_if in conn.dis_path],
                    "dis_QoS": dict(conn.dis_QoS) if conn.dis_QoS else {},
                })
            except Exception as e:
                connections_snapshot.append({
                    "id": getattr(conn, "request_id", None),
                    "error": str(e),
                })
